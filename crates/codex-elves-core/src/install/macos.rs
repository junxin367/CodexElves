...
fn copy_icon(resources: &Path) -> anyhow::Result<()> {
    let source = std::env::current_exe()
        ...
        .map(|path| path.join("codex-elves.png"));
    if let Some(source) = source.filter(|path| path.exists()) {
        fs::copy(source, resources.join("codex-elves.png"))?;
    }
    Ok(())
}
...
<dict>
  ...
  <string>com.bigpizzav3.codexelves{identifier_suffix}</string>
