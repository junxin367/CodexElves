...
const UNINSTALL_SUBKEY: &str = r"Software\Microsoft\Windows\CurrentVersion\Uninstall\CodexElves";
...
pub fn build_windows_entrypoint_plan(options: &InstallOptions) -> WindowsEntrypointPlan {
    ...
    WindowsEntrypointPlan {
        ...
        uninstall_key: "CodexElves".to_string(),
...
fn write_uninstall_registration(plan: &WindowsEntrypointPlan) -> anyhow::Result<()> {
    let install_location = Path::new(&plan.manager_path)
        ...
        .to_string();
    for (name, value) in [
        ("DisplayName", "CodexElves".to_string()),
        ("DisplayVersion", crate::version::VERSION.to_string()),
        ("Publisher", "junxin367".to_string()),
...
fn default_icon_path() -> PathBuf {
    std::env::current_exe()
        ...
        .map(|path| path.join("codex-elves.ico"))
        .unwrap_or_else(|| PathBuf::from("codex-elves.ico"))
}
