...
fn wrap_script(script: &UserScriptFile, source: &str) -> String {
    format!(
        r#"
(() => {{
  window.__codexElvesUserScripts = window.__codexElvesUserScripts || {{ scripts: {{}} }};
  const key = {key};
  window.__codexElvesUserScripts.scripts[key] = {{ key, name: {name}, source: {source_name}, status: "loading", error: "", loadedAt: new Date().toISOString() }};
  try {{
{source}
    window.__codexElvesUserScripts.scripts[key].status = "loaded";
    window.__codexElvesUserScripts.scripts[key].loadedAt = new Date().toISOString();
  }} catch (error) {{
    window.__codexElvesUserScripts.scripts[key].status = "failed";
    window.__codexElvesUserScripts.scripts[key].error = String(error && (error.stack || error.message) || error);
  }}
}})();
"#,
