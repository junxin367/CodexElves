...
pub const DEFAULT_REPOSITORY: &str = "junxin367/CodexElves";
pub const DEFAULT_LATEST_JSON_URL: &str =
    "https://github.com/junxin367/CodexElves/releases/latest/download/latest.json";
...
fn platform_asset_rank(name: &str) -> u8 {
    // 0 = current OS + native arch.
    // 1 = current OS + other arch, used only as a fallback.
    // 2 = wrong platform or unsupported asset.
...
fn is_windows_installer_asset(name: &str) -> bool {
    name.contains("codex")
        && (name.contains("elves") || name.contains("plus"))
        && (name.ends_with(".msi")
