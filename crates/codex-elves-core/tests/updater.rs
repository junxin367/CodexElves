...
fn github_payload_selects_platform_installer() {
    let release = release_from_github_payload(&json!({
        "tag_name": "v1.0.9",
        "html_url": "https://github.com/junxin367/CodexElves/releases/tag/v1.0.9",
        "body": "fixes",
        "assets": [
            {"name": "source.zip", "browser_download_url": "https://example.test/source.zip"},
            {"name": "codex-elves-manager.exe", "browser_download_url": "https://example.test/manager.exe"},
            {"name": "CodexElves_1.0.9_x64-setup.exe", "browser_download_url": "https://example.test/setup.exe"},
            {"name": "CodexElves_1.0.9_x64.dmg", "browser_download_url": "https://example.test/app.dmg"}
        ]
    }))
    .unwrap();
    ...
    if cfg!(windows) {
        assert_eq!(
            ...
            Some("CodexElves_1.0.9_x64-setup.exe")
        );
    } else if cfg!(target_os = "macos") {
        assert_eq!(
            release.asset_name.as_deref(),
            Some("CodexElves_1.0.9_x64.dmg")
        );
    } else {
...
fn latest_json_payload_selects_platform_installer_without_github_api_shape() {
    let release = release_from_latest_json_payload(&json!({
        "version": "v1.1.6",
        "url": "https://github.com/junxin367/CodexElves/releases/tag/v1.1.6",
        "body": "静态更新描述",
        "assets": [
            {"name": "source.zip", "url": "https://example.test/source.zip"},
            {"name": "CodexElves-1.1.6-windows-x64-setup.exe", "url": "https://example.test/setup.exe"},
            {"name": "CodexElves-1.1.6-macos-x64.dmg", "url": "https://example.test/app.dmg"}
        ]
    }))
    .unwrap();
    ...
    if cfg!(windows) {
        assert_eq!(
            release.asset_name.as_deref(),
            Some("CodexElves-1.1.6-windows-x64-setup.exe")
        );
    } else if cfg!(target_os = "macos") {
        assert_eq!(
            release.asset_name.as_deref(),
            Some("CodexElves-1.1.6-macos-x64.dmg")
        );
    } else {
        assert_eq!(release.asset_name.as_deref(), None);
    }
...
fn asset_selection_prefers_current_platform_artifacts() {
    let assets = vec![
        (
            "CodexElves.zip".to_string(),
            "https://example.test/source.zip".to_string(),
        ),
        (
            "codex-elves-manager.exe".to_string(),
            "https://example.test/manager.exe".to_string(),
        ),
        (
            "CodexElves_1.0.9_x64-setup.exe".to_string(),
            "https://example.test/setup.exe".to_string(),
        ),
        (
            "CodexElves_1.0.9_x64.dmg".to_string(),
    ...
    if cfg!(windows) {
        ...
        assert_eq!(selected.name, "CodexElves_1.0.9_x64-setup.exe");
    } else if cfg!(target_os = "macos") {
        let selected = select_update_asset(&assets).unwrap();
        assert_eq!(selected.name, "CodexElves_1.0.9_x64.dmg");
    } else {
...
fn asset_selection_distinguishes_x64_and_arm64_macos_dmgs() {
    let assets = vec![
        (
            "CodexElves-1.2.17-macos-arm64.dmg".to_string(),
            "https://example.test/app-arm64.dmg".to_string(),
        ),
        (
            "CodexElves-1.2.17-macos-x64.dmg".to_string(),
    ...
    if cfg!(target_os = "macos") {
        ...
        let expected = match std::env::consts::ARCH {
            "x86_64" => "CodexElves-1.2.17-macos-x64.dmg",
            "aarch64" => "CodexElves-1.2.17-macos-arm64.dmg",
