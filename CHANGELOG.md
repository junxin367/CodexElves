...
- 修复设置页返回会话后 Token 用量卡片消失的问题，卡片会重新绑定置顶摘要并立即恢复缓存数据。
- 修正子智能体 Token 在父会话中的重复累计，确保会话总量与最近一轮统计准确。
- 模型配置新增“自动添加模型”，可从供应商接口批量拉取并按当前协议补充未配置模型。
- 将 Rust workspace、npm、Tauri 与前端版本元数据升级到 `0.3.2`。

## 0.3.1 - 2026-07-22

- 新增单会话 Token 用量统计，可展示累计与最近一轮的输入、输出、缓存消耗，并递归计入子智能体。
- Token 数据支持 `K`、`M`、`B` 单位，运行中按通知即时刷新并保留 2.5 秒兜底校准；功能默认关闭。
- Token 卡片紧凑显示在置顶摘要下方，与原生摘要同步显示和隐藏；隐藏期间保留当前数据并暂停刷新。
- Codex Radar 改为读取结构化 `current.json` 数据，并在读取失败时直接展示错误信息。
- 代理日志响应详情兼容 JSON Lines 事件流，优先提取最终响应内容。
- 将 Rust workspace、npm、Tauri 与前端版本元数据升级到 `0.3.1`。
