...
- `cargo test -p codex-elves-core bridge_routes --manifest-path Cargo.toml`
- `cargo test -p codex-elves-data storage_adapter --manifest-path Cargo.toml`

## 注意事项

该能力是 `Codex Token Usage 0.1.6` 的配套宿主支持。若仅更新脚本市场仓、未更新 `CodexElves`，则“重启后按历史恢复统计”不会完整生效。
