...
- 修改 `apps/codex-elves-manager/src-tauri/src/commands.rs`
  - 增加上下文管理命令和命令层测试。
  - 切换供应商时传入供应商上下文选择和上下文 token 字段。
- 修改 `apps/codex-elves-manager/src-tauri/src/lib.rs`
  - 注册新增 Tauri commands。
- 修改 `apps/codex-elves-manager/src/App.tsx`
  - 增加 TS 类型、默认值、normalize 兼容逻辑、供应商上下文 UI 和新增供应商默认全选逻辑。
- 修改 `apps/codex-elves-manager/src/styles.css`
  - 增加上下文管理区布局和行样式。
...
- Modify: `apps/codex-elves-manager/src/App.tsx`
...
在 `apps/codex-elves-manager/src/App.tsx` 增加：
...
```powershell
git add crates/codex-plus-core/src/settings.rs apps/codex-elves-manager/src/App.tsx
...
**Files:**
- Modify: `apps/codex-elves-manager/src-tauri/src/commands.rs`
- Modify: `apps/codex-elves-manager/src-tauri/src/lib.rs`
- Test: `apps/codex-elves-manager/src-tauri/src/commands.rs`
...
```powershell
cargo test -p codex-elves-manager context_entry_commands_update_settings_payload
```
...
在 `apps/codex-elves-manager/src-tauri/src/lib.rs` 的 `generate_handler!` 中加入：
...
```powershell
cargo test -p codex-elves-manager context_entry_commands_update_settings_payload
cargo test -p codex-elves-manager
```
...
```powershell
git add apps/codex-elves-manager/src-tauri/src/commands.rs apps/codex-elves-manager/src-tauri/src/lib.rs
...
**Files:**
- Modify: `apps/codex-elves-manager/src/App.tsx`
- Modify: `apps/codex-elves-manager/src/styles.css`
...
```powershell
git add apps/codex-elves-manager/src/App.tsx apps/codex-elves-manager/src/styles.css
...
```powershell
git add apps/codex-elves-manager/src/App.tsx
...
Working directory: `apps/codex-elves-manager`
...
```powershell
git add crates/codex-plus-core/src/settings.rs crates/codex-plus-core/src/relay_config.rs crates/codex-plus-core/tests/relay_config.rs apps/codex-elves-manager/src-tauri/src/commands.rs apps/codex-elves-manager/src-tauri/src/lib.rs apps/codex-elves-manager/src/App.tsx apps/codex-elves-manager/src/styles.css
